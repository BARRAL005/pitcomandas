...
import { Product, Comanda, ActivityLog, Customer, ComandaItem, ConsignmentOrder } from './types';
...
import ConsignacoesView from './components/ConsignacoesView';
...
export default function App() {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    if (saved) {
      ...
      const hasConsignment = parsed.some(p => p.category === 'Consignados' || p.id === 'p26');
      if (!hasConsignment) {
        const newItems = INITIAL_PRODUCTS.filter(ip => !parsed.some(p => p.id === ip.id));
